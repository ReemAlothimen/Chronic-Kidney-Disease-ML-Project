...
# Visualizing the logistic regression result on the Test set
...
# Visualizing the logistic regression result on the training set
X1_train, X2_train = np.meshgrid(np.arange(start=X_train[:, 0].min() - 1, stop=X_train[:, 0].max() + 1, step=0.01),
                                 np.arange(start=X_train[:, 1].min() - 1, stop=X_train[:, 1].max() + 1, step=0.01))

# Use the trained logistic regression model to predict on the meshgrid
Z_train = LogRegmodel.predict(np.array([X1_train.ravel(), X2_train.ravel()]).T)
Z_train = Z_train.reshape(X1_train.shape)

# Create a color map for the plot
colors_train = ListedColormap(('orange', 'blue'))

# Plot the contourf plot
plt.contourf(X1_train, X2_train, Z_train, alpha=0.5, cmap=colors_train)

# Plot the scatter plot of the training set
plt.scatter(X_train[y_train == 0, 0], X_train[y_train == 0, 1], color='blue', label='notckd')
plt.scatter(X_train[y_train == 1, 0], X_train[y_train == 1, 1], color='orange', label='ckd')
...
# Use the trained logistic regression model to predict on the meshgrid
Z = LogRegmodel.predict(np.array([X1.ravel(), X2.ravel()]).T)
...
# Plot the contourf plot
plt.contourf(X1, X2, Z, alpha=0.5, cmap=colors)

# Plot the scatter plot of the test set
plt.scatter(X_test[y_test == 0, 0], X_test[y_test == 0, 1], color='green', label='notckd')
plt.scatter(X_test[y_test == 1, 0], X_test[y_test == 1, 1], color='red', label='ckd')
